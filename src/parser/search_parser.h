...
switch (yykind)
    {
      ...
      {
        return SearchParser::symbol_name (this->kind ());
      }
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
...
#endif
      {
        ...
        YY_ASSERT (tok == token::END
                   || (token::YYerror <= tok && tok <= token::OP_COLON));
#endif
      }
#if 201103L <= YY_CPLUSPLUS
...
#if 201103L <= YY_CPLUSPLUS
    /// Non copyable.
    SearchParser (const SearchParser&) = delete;
    /// Non copyable.
    SearchParser& operator= (const SearchParser&) = delete;
#endif
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_OP_COLON (location_type l)
      {
        return symbol_type (token::OP_COLON, std::move (l));
      }
#else
      static
      symbol_type
      make_OP_COLON (const location_type& l)
      {
        return symbol_type (token::OP_COLON, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_CARAT (const float& v, const location_type& l)
      {
        return symbol_type (token::CARAT, v, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
    ...
    {
    public:
      context (const SearchParser& yyparser, const symbol_type& yyla);
...
#if YY_CPLUSPLUS < 201103L
    /// Non copyable.
    SearchParser (const SearchParser&);
    /// Non copyable.
    SearchParser& operator= (const SearchParser&);
#endif
