...
std::variant<UniquePtr<QueryContext>, QueryResult> hybridsearch::GetQueryContext(bool is_admin_stmt, bool is_admin_show_node) const {
    hybridsearchContext &context = hybridsearchContext::instance();
    if (!context.hybridsearchContextInited()) {
        QueryResult query_result;
        query_result.result_table_ = nullptr;
        query_result.status_ = Status::hybridsearchIsIniting();
        return query_result;
    }
    bool check_start = !is_admin_stmt || (!context.IsAdminRole() && !is_admin_show_node); // refactor by old code
    bool error = check_start && !context.hybridsearchContextStarted();
    if (error) {
        QueryResult query_result;
        query_result.result_table_ = nullptr;
...
u64 hybridsearch::GetSessionId() { return session_->session_id(); }

void hybridsearch::Hello() { fmt::print("hello hybridsearch\n"); }

void hybridsearch::LocalInit(const String &path, const String &config_path) {
    if (!config_path.empty() && VirtualStore::Exists(config_path)) {
        SharedPtr<String> config_path_ptr = MakeShared<String>(config_path);
        hybridsearchContext::instance().InitPhase1(config_path_ptr);
    } else {
        LOG_WARN(fmt::format("hybridsearch::LocalInit cannot find config: {}", config_path));
        ...
        hybridsearchContext::instance().InitPhase1(nullptr, default_config.get());
    }
    hybridsearchContext::instance().InitPhase2();
}

void hybridsearch::LocalUnInit() { hybridsearchContext::instance().UnInit(); }

SharedPtr<hybridsearch> hybridsearch::LocalConnect() {
    SharedPtr<hybridsearch> hybridsearch_ptr = MakeShared<hybridsearch>();

    SessionManager *session_mgr = hybridsearchContext::instance().session_manager();
    hybridsearch_ptr->session_ = session_mgr->CreateLocalSession();
    return hybridsearch_ptr;
}

void hybridsearch::LocalDisconnect() {
    //    fmt::print("To disconnect the database.\n");
}

SharedPtr<hybridsearch> hybridsearch::RemoteConnect() {
    SharedPtr<hybridsearch> hybridsearch_ptr = MakeShared<hybridsearch>();
    SessionManager *session_mgr = hybridsearchContext::instance().session_manager();
    ...
    hybridsearch_ptr->session_ = std::move(remote_session);
    return hybridsearch_ptr;
}

void hybridsearch::RemoteDisconnect() {
    SessionManager *session_mgr = hybridsearchContext::instance().session_manager();
    session_mgr->RemoveSessionByID(session_->session_id());
    session_.reset();
}

QueryResult hybridsearch::CreateDatabase(const String &schema_name, const CreateDatabaseOptions &create_db_options, const String &comment) {
...
QueryResult hybridsearch::DropDatabase(const String &schema_name, const DropDatabaseOptions &drop_database_options) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::ListDatabases() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::GetDatabase(const String &schema_name) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::ShowDatabase(const String &schema_name) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
...
QueryResult hybridsearch::Query(const String &query_text) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::Flush(const String &flush_type) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::Compact(const String &db_name, const String &table_name) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::SetVariableOrConfig(const String &name, bool value, SetScope scope) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::SetVariableOrConfig(const String &name, i64 value, SetScope scope) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::SetVariableOrConfig(const String &name, double value, SetScope scope) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::SetVariableOrConfig(const String &name, String value, SetScope scope) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::ShowVariable(const String &variable_name, SetScope scope) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::ShowVariables(SetScope scope) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::ShowConfig(const String &config_name) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::ShowConfigs() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::CreateTable(const String &db_name,
...
QueryResult hybridsearch::DropTable(const String &db_name, const String &table_name, const DropTableOptions &options) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<DropStatement> drop_statement = MakeUnique<DropStatement>();
...
QueryResult hybridsearch::ListTables(const String &db_name) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
...
QueryResult hybridsearch::ShowTable(const String &db_name, const String &table_name) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
    show_statement->schema_name_ = db_name;
...
QueryResult hybridsearch::ShowColumns(const String &db_name, const String &table_name) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
    show_statement->schema_name_ = db_name;
...
QueryResult hybridsearch::ShowTables(const String &db_name) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
    show_statement->schema_name_ = db_name;
...
QueryResult hybridsearch::GetTable(const String &db_name, const String &table_name) {
    UniquePtr<QueryContext> query_context_ptr = MakeUnique<QueryContext>(session_.get());
...
QueryResult hybridsearch::ListTableIndexes(const String &db_name, const String &table_name) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
    show_statement->schema_name_ = db_name;
...
QueryResult hybridsearch::CreateIndex(const String &db_name,
                                  const String &table_name,
...
QueryResult
hybridsearch::DropIndex(const String &db_name, const String &table_name, const String &index_name, const DropIndexOptions &drop_index_options) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<DropStatement> drop_statement = MakeUnique<DropStatement>();
...
QueryResult hybridsearch::ShowIndex(const String &db_name, const String &table_name, const String &index_name) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
    show_statement->schema_name_ = db_name;
...
QueryResult hybridsearch::ShowIndexSegment(const String &db_name, const String &table_name, const String &index_name, SegmentID segment_id) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
    show_statement->schema_name_ = db_name;
...
QueryResult
hybridsearch::ShowIndexChunk(const String &db_name, const String &table_name, const String &index_name, SegmentID segment_id, ChunkID chunk_id) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
    show_statement->schema_name_ = db_name;
...
QueryResult hybridsearch::ShowSegment(const String &db_name, const String &table_name, const SegmentID &segment_id) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
    show_statement->schema_name_ = db_name;
...
QueryResult hybridsearch::ShowSegments(const String &db_name, const String &table_name) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
    show_statement->schema_name_ = db_name;
...
QueryResult hybridsearch::ShowBlock(const String &db_name, const String &table_name, const SegmentID &segment_id, const BlockID &block_id) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
    show_statement->schema_name_ = db_name;
...
QueryResult hybridsearch::ShowBlocks(const String &db_name, const String &table_name, const SegmentID &segment_id) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
    show_statement->schema_name_ = db_name;
...
QueryResult hybridsearch::ShowBlockColumn(const String &db_name,
                                      const String &table_name,
...
QueryResult hybridsearch::ShowBuffer() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
...
QueryResult hybridsearch::ShowProfiles() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
...
QueryResult hybridsearch::ShowMemindex() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
...
QueryResult hybridsearch::ShowQueries() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
...
QueryResult hybridsearch::ShowQuery(u64 query_index) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
    show_statement->show_type_ = ShowStmtType::kQueries;
...
QueryResult hybridsearch::ShowTransactions() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
    show_statement->show_type_ = ShowStmtType::kQueries;
...
QueryResult hybridsearch::ShowLogs() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
...
QueryResult hybridsearch::ShowDeltaCheckpoint() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
...
QueryResult hybridsearch::ShowFullCheckpoint() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
...
QueryResult hybridsearch::ShowObjects() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
...
QueryResult hybridsearch::ShowObject(const String &filename) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
...
QueryResult hybridsearch::ShowFilesInObject() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
...
QueryResult hybridsearch::ShowMemory() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
...
QueryResult hybridsearch::ShowMemoryObjects() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
...
QueryResult hybridsearch::ShowMemoryAllocations() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
...
QueryResult hybridsearch::ShowFunction(const String &function_name) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
    UniquePtr<ShowStatement> show_statement = MakeUnique<ShowStatement>();
...
QueryResult hybridsearch::Insert(const String &db_name, const String &table_name, Vector<InsertRowExpr *> *insert_rows) {
...
QueryResult hybridsearch::Import(const String &db_name, const String &table_name, const String &path, ImportOptions import_options) {
...
QueryResult
hybridsearch::Export(const String &db_name, const String &table_name, Vector<ParsedExpr *> *columns, const String &path, ExportOptions export_options) {
...
QueryResult hybridsearch::Delete(const String &db_name, const String &table_name, ParsedExpr *filter) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::Update(const String &db_name, const String &table_name, ParsedExpr *filter, Vector<UpdateExpr *> *update_list) {
...
QueryResult hybridsearch::Explain(const String &db_name,
                              const String &table_name,
...
QueryResult hybridsearch::Search(const String &db_name,
                             const String &table_name,
                             SearchExpr *search_expr,
                             ParsedExpr *filter,
                             ParsedExpr *limit,
...
QueryResult hybridsearch::Optimize(const String &db_name, const String &table_name, OptimizeOptions optimize_option) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::AddColumns(const String &db_name, const String &table_name, Vector<SharedPtr<ColumnDef>> column_defs) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::DropColumns(const String &db_name, const String &table_name, Vector<String> column_names) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::Cleanup() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::ForceCheckpoint() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);

    auto flush_statement = MakeUnique<FlushStatement>();
    flush_statement->type_ = hybridsearch::FlushType::kData;
...
QueryResult hybridsearch::CompactTable(const String &db_name, const String &table_name) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::TestCommand(const String &command_content) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::AdminShowCatalogs() {
    UniquePtr<QueryContext> query_context_ptr;
...
QueryResult hybridsearch::AdminShowCatalog(i64 index) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(true), query_context_ptr);
...
QueryResult hybridsearch::AdminShowLogs() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(true), query_context_ptr);
...
QueryResult hybridsearch::AdminShowLog(i64 index) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(true), query_context_ptr);
...
QueryResult hybridsearch::AdminShowConfigs() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(true), query_context_ptr);
...
QueryResult hybridsearch::AdminShowVariables() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(true), query_context_ptr);
...
QueryResult hybridsearch::AdminShowVariable(String var_name) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(true), query_context_ptr);
...
QueryResult hybridsearch::AdminShowNodes() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(), query_context_ptr);
...
QueryResult hybridsearch::AdminShowNode(String node_name) {
    UniquePtr<QueryContext> query_context_ptr;
...
QueryResult hybridsearch::AdminShowCurrentNode() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(true), query_context_ptr);
...
QueryResult hybridsearch::AdminRemoveNode(String node_name) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(true), query_context_ptr);
...
QueryResult hybridsearch::AdminSetAdmin() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(true), query_context_ptr);
...
QueryResult hybridsearch::AdminSetStandalone() {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(true), query_context_ptr);
...
QueryResult hybridsearch::AdminSetLeader(String node_name) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(true), query_context_ptr);
...
QueryResult hybridsearch::AdminSetFollower(String node_name, const String &leader_address) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(true), query_context_ptr);
...
QueryResult hybridsearch::AdminSetLearner(String node_name, const String &leader_address) {
    UniquePtr<QueryContext> query_context_ptr;
    GET_QUERY_CONTEXT(GetQueryContext(true), query_context_ptr);
