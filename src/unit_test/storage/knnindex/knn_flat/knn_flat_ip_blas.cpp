...
TEST_F(KnnFlatIpBlasTest, test1) {
    using namespace hybridsearch;
    ...
    KnnFlatIPBlas<f32> knn_distance(query_embedding.get(), 1, top_k, dimension, EmbeddingDataType::kElemFloat);
...

    {
        ...
        p_bitmask->SetFalse(0);
        {
            ...
            EXPECT_FLOAT_EQ(distance_array_m[0], 0.26);
            EXPECT_FLOAT_EQ(id_array_m[0].segment_id_, 0);
