...
class DayOfWeekFunctionsTest : public BaseTestParamStr {};

INSTANTIATE_TEST_SUITE_P(TestWithDifferentParams, DayOfWeekFunctionsTest, ::testing::Values(BaseTestParamStr::NULL_CONFIG_PATH));

TEST_P(DayOfWeekFunctionsTest, day_of_week_func) {
    using namespace hybridsearch;

    UniquePtr<Catalog> catalog_ptr = MakeUnique<Catalog>();

    RegisterDayOfWeekFunction(catalog_ptr);
...

    {
        ...
        EXPECT_STREQ("dayofweek(Date)->BigInt", func.ToString().c_str());
        ...
        ScalarFunction func = scalar_function_set->GetMostMatchFunction(inputs);
        EXPECT_STREQ("dayofweek(DateTime)->BigInt", func.ToString().c_str());
...

    {
        ...
        ScalarFunction func = scalar_function_set->GetMostMatchFunction(inputs);
        EXPECT_STREQ("dayofweek(Timestamp)->BigInt", func.ToString().c_str());
