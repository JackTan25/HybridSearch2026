...
class SecondFunctionsTest : public BaseTestParamStr {};

INSTANTIATE_TEST_SUITE_P(TestWithDifferentParams, SecondFunctionsTest, ::testing::Values(BaseTestParamStr::NULL_CONFIG_PATH));

TEST_P(SecondFunctionsTest, second_func) {
    using namespace hybridsearch;

    UniquePtr<Catalog> catalog_ptr = MakeUnique<Catalog>();

    RegisterSecondFunction(catalog_ptr);
...

    {
        ...
        EXPECT_STREQ("second(DateTime)->BigInt", func.ToString().c_str());
...

    {
        ...
        ScalarFunction func = scalar_function_set->GetMostMatchFunction(inputs);
        EXPECT_STREQ("second(Time)->BigInt", func.ToString().c_str());
...

    {
        ...
        ScalarFunction func = scalar_function_set->GetMostMatchFunction(inputs);
        EXPECT_STREQ("second(Timestamp)->BigInt", func.ToString().c_str());
