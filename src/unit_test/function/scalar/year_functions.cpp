...
class YearFunctionsTest : public BaseTestParamStr {};

INSTANTIATE_TEST_SUITE_P(TestWithDifferentParams, YearFunctionsTest, ::testing::Values(BaseTestParamStr::NULL_CONFIG_PATH));

TEST_P(YearFunctionsTest, year_func) {
    using namespace hybridsearch;

    UniquePtr<Catalog> catalog_ptr = MakeUnique<Catalog>();

    RegisterYearFunction(catalog_ptr);
...

    {
        ...
        EXPECT_STREQ("year(Date)->BigInt", func.ToString().c_str());
        ...
        ScalarFunction func = scalar_function_set->GetMostMatchFunction(inputs);
        EXPECT_STREQ("year(DateTime)->BigInt", func.ToString().c_str());
...

    {
        ...
        ScalarFunction func = scalar_function_set->GetMostMatchFunction(inputs);
        EXPECT_STREQ("year(Timestamp)->BigInt", func.ToString().c_str());
