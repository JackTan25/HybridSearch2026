...
class DayOfYearFunctionsTest : public BaseTestParamStr {};

INSTANTIATE_TEST_SUITE_P(TestWithDifferentParams, DayOfYearFunctionsTest, ::testing::Values(BaseTestParamStr::NULL_CONFIG_PATH));

TEST_P(DayOfYearFunctionsTest, day_of_year_func) {
    using namespace hybridsearch;

    UniquePtr<Catalog> catalog_ptr = MakeUnique<Catalog>();

    RegisterDayOfYearFunction(catalog_ptr);
...

    {
        ...
        EXPECT_STREQ("dayofyear(Date)->BigInt", func.ToString().c_str());
        ...
        ScalarFunction func = scalar_function_set->GetMostMatchFunction(inputs);
        EXPECT_STREQ("dayofyear(DateTime)->BigInt", func.ToString().c_str());
...

    {
        ...
        ScalarFunction func = scalar_function_set->GetMostMatchFunction(inputs);
        EXPECT_STREQ("dayofyear(Timestamp)->BigInt", func.ToString().c_str());
