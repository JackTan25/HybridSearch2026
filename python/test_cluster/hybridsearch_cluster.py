...
from hybridsearch_http import hybridsearch_http, http_network_util
...
class hybridsearchRunner(BasehybridsearchRunner):
    def __init__(
        self,
        node_name: str,
        executable_path: str,
    ...
    def add_client(self, http_addr: str):
        ...
        net.set_retry()
        self.client = hybridsearch_http(net=net)
...
class hybridsearchCluster:
    def __init__(
        self, executable_path: str, *, test_name: str = None
    ):
        self.executable_path = executable_path
        self.runners: dict[str, hybridsearchRunner] = {}
        self.leader_runner: hybridsearchRunner | None = None
    ...
    def client(self, node_name: str) -> hybridsearch_http | None:
    ...
    def remove_node(self, node_name: str, kill: bool = False):
        ...
        if kill and self.leader_runner is not None:
            hybridsearch_leader = self.leader_runner.client
            wait_timeout_sec = 10
            start_time = time.time()
            while time.time() - start_time < wait_timeout_sec:
                time.sleep(0.5)
                node_info = hybridsearch_leader.show_node(node_name)
            ...
            hybridsearch_leader.remove_node(node_name)
            self.logger.debug(
                f"Remove node {node_name} from the cluster because status is {node_info.node_status}."
            )

    def _get_runner(self, node_name: str) -> hybridsearchRunner:
