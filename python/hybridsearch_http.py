...
from hybridsearch.common import ConflictType, hybridsearchException, SparseVector, SortType
            ...
            if resp.status_code != 200:
                if resp.status_code == 500:
                    ...
                    if "error_message" in resp_json:
                        ...
                        raise hybridsearchException(resp_json["error_code"], resp_json["error_message"])
                    else:
                        print(500, resp_json["error_code"])
                        raise hybridsearchException(resp_json["error_code"], "")
...
class hybridsearch_http:
    ...
    def create_database(self, db_name, opt=ConflictType.Error):
        ...
        if opt in [ConflictType.Error, ConflictType.Ignore, ConflictType.Replace]:
            ...
            try:
                ...
                raise hybridsearchException(ErrorCode.INVALID_CONFLICT_TYPE)
            # d = self.net.set_up_data(
            #    ["create_option"], {"create_option": str(opt)}
            # )
            # different exception type
            # <ExceptionInfo hybridsearchException(3074, 'Invalid create option: 1.1') tblen=3>
    ...
    def get_database(self, db_name, opt=ConflictType.Error):
        ...
        try:
            ...
            raise hybridsearchException(ErrorCode.DB_NOT_EXIST)
...
class database_http:
    ...
    def create_table(
        ...
        try:
            ...
            raise hybridsearchException(ErrorCode.SYNTAX_ERROR, "http adapter create table parse error")
        # print(fields)
        ...
        if index_info.params is not None:
            for key, value in index_info.params.items():
                if not isinstance(value, str):
                    raise hybridsearchException(ErrorCode.INVALID_INDEX_PARAM, f"parameter value: {value} isn't string")
    ...
    def sort(self, order_by_expr_list: Optional[List[list[str, SortType]]]):
        for order_by_expr in order_by_expr_list:
            ...
            if len(order_by_expr) != 2:
                raise hybridsearchException(ErrorCode.INVALID_PARAMETER,
                                        "order_by_expr_list must be a list of [column_name, sort_type]")
            if order_by_expr[1] not in [SortType.Asc, SortType.Desc]:
                raise hybridsearchException(ErrorCode.INVALID_PARAMETER, "sort_type must be SortType.Asc or SortType.Desc")
