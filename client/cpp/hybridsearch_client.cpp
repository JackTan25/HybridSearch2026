...
Client Client::Connect(const std::string &ip_address, uint16_t port) {
    ...
    std::unique_ptr<hybridsearchServiceClient> client = std::make_unique<hybridsearchServiceClient>(protocol);
    transport->open();
...
ClientStatus Client::CreateDatabase(const std::string &db_name, CreateOption create_option) {
    ...
    switch (create_option) {
        case CreateOption::kErrorIfExists: {
            request.create_option.conflict_type = hybridsearch_thrift_rpc::CreateConflict::type::Error;
            break;
        }
        case CreateOption::kIgnoreIfExists: {
            request.create_option.conflict_type = hybridsearch_thrift_rpc::CreateConflict::type::Ignore;
            break;
        }
        case CreateOption::kReplaceIfExists: {
            request.create_option.conflict_type = hybridsearch_thrift_rpc::CreateConflict::type::Replace;
            break;
        }
        default: {
...
ClientStatus Client::DropDatabase(const std::string &db_name, DropOption drop_option) {
    ...
    switch (drop_option) {
        case DropOption::kErrorIfNotExists: {
            request.drop_option.conflict_type = hybridsearch_thrift_rpc::DropConflict::type::Error;
            break;
        }
        case DropOption::kIgnoreIfNotExists: {
            request.drop_option.conflict_type = hybridsearch_thrift_rpc::DropConflict::type::Ignore;
            break;
        }
        default: {
            return ClientStatus{ClientErrorCode::kInvalidConflictType, "invalid conflict type"};
