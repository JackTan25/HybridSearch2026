...
How to set and load configuration file when starting hybridsearch.

---

This document provides instructions for loading configuration file for hybridsearch and descriptions of each configuration entry.
...
<Tabs
  ...
  values={[
    ...
    {label: 'Embedded hybridsearch', value: 'embedded'},
  ]}>
  <TabItem value="docker">

To load a configuration file when you start up hybridsearch using Docker:
...
2. When running Docker, specify the configuration file path using the `-f` option. For example: 
   ...
   ```bash
   docker run -d --name hybridsearch -v /var/hybridsearch/:/var/hybridsearch --ulimit nofile=500000:500000 --network=host hybridsearchflow/hybridsearch:nightly -f /var/hybridsearch/hybridsearch_conf.toml
   ```

</TabItem>
  <TabItem value="binary">

If you start up hybridsearch from DEB or RPM, the configuration file will be in the `/usr/etc` directory. When the hybridsearch service starts, it will load this configuration file.

</TabItem>
  <TabItem value="embedded">

hybridsearch embedded does not have an *explicit* configuration file. Its configuration file should meet the following requirements:

- It is in the local folder where hybridsearch embedded is opened (the path that you specify when calling `connect()`), for example `/var/hybridsearch/`:
- Its name must exactly match **hybridsearch_conf.toml**.
...
```toml
[general]
# hybridsearch server version, the minor version of this parameter should match server minor version.
version                  = "0.6.0"
# Timezone
time_zone                = "utc-8"
...
# - `"standalone"`: Start hybridsearch as a standalone server.
# - `"admin"`:
#   - Start hybridsearch either as a standalone server in `ADMIN` mode (when `storage_type` is set to `"local"`)
#   - Start hybridsearch as a cluster node in `ADMIN` mode (when `storage_type` is set to `"minio"`)
server_mode              = "standalone"

# Network configuration
[network]
# IP address of the hybridsearch server
...
# The delay time for reconnecting to the hybridsearch peer server after a failed connection
peer_retry_delay         = 0
# The maximum number of reconnection attempts to the hybridsearch peer server after a failed connection
peer_retry_count           = 0
# Timeout for connecting to the hybridsearch peer server
peer_connect_timeout     = 2000
# Timeout for the hybridsearch peer server to receive data
peer_recv_timeout        = 0
# Timeout for the hybridsearch peer server to send data
peer_send_timeout        = 0
...
# - Holds the data that hybridsearch saves when using the local disk for storage.
...
# If same query is sent to hybridsearch, hybridsearch will check and return the cached result.
result_cache            = "on"
...
[resource]
# Directory for hybridsearch's resource files, including the dictionary files used by the analyzer
