...
Build hybridsearch from source, build and run unit/functional tests.

---

This document provides instructions for building hybridsearch from source, as well as building and running unit and functional tests. 

:::tip NOTE
hybridsearch can be natively compiled on Linux only. For non-Linux operating systems, build it using Docker.
:::
...
- `Release`: Suitable for [project releases](https://github.com/hybridsearchflow/hybridsearch/releases).
...
This section provides instructions for building hybridsearch from source on Linux using Docker.

1. Download the source code:

```shell
git clone https://github.com/hybridsearchflow/hybridsearch.git
```

2. Build the source code using Docker:

```shell
cd hybridsearch && mkdir cmake-build-debug
TZ=$(readlink -f /etc/localtime | awk -F '/zoneinfo/' '{print $2}')
docker run -d --name hybridsearch_build -e TZ=$TZ -v $PWD:/hybridsearch -v /boot:/boot hybridsearchflow/hybridsearch_builder:centos7_clang18
docker exec hybridsearch_build bash -c "cd /hybridsearch/cmake-build-debug && cmake -G Ninja -DCMAKE_BUILD_TYPE=Debug -DCMAKE_VERBOSE_MAKEFILE=ON .. && cmake --build . -t  hybridsearch"
```

3. Start up the hybridsearch server:

```shell
sudo mkdir -p /var/hybridsearch && sudo chown -R $USER /var/hybridsearch
ulimit -n 500000
./cmake-build-debug/src/hybridsearch
```

  </TabItem>
  <TabItem value="ubuntu">

This section provides instructions for building hybridsearch from source on Ubuntu.
...
```shell
git config --global --add safe.directory hybridsearch
cd hybridsearch && mkdir cmake-build-debug && cd cmake-build-debug
export CC=/usr/bin/clang-18
export CXX=/usr/bin/clang++-18
cmake -G Ninja -DCMAKE_BUILD_TYPE=Debug -DCMAKE_VERBOSE_MAKEFILE=ON ..
cmake --build . -t hybridsearch
```

4. Start up the hybridsearch server:
...
```shell
docker exec hybridsearch_build bash -c "cd /hybridsearch/cmake-build-debug && cmake -G Ninja -DCMAKE_BUILD_TYPE=Debug -DCMAKE_VERBOSE_MAKEFILE=ON .. && cmake --build . -t  test_main"
...
3. Use Gcovr to generate summarized code coverage results:
   ...
   gcovr --gcov-executable "llvm-cov gcov" -r "YOUR_ABSOLUTE_PATH_OF_THE_PROJECT/hybridsearch/src" --gcov-exclude-directories ${PWD}'/unit_test' . --html unit_test_html.html
   ```
...
1. Build and start up the hybridsearch server:
...
2. Install Python sdk of hybridsearch:
   ```bash
...
1. Build and start up the hybridsearch server with code coverage enabled:
...
4. Shut down the hybridsearch server:

   ```shell
   kill -15 `pidof hybridsearch`
   ```
...
5. Use Gcovr to generate summarized code coverage results:
   ...
   gcovr --gcov-executable "llvm-cov gcov" -r "YOUR_ABSOLUTE_PATH_OF_THE_PROJECT/hybridsearch/src" --gcov-exclude-directories ${PWD}'/unit_test' . --html function_test_html.html
   ```
