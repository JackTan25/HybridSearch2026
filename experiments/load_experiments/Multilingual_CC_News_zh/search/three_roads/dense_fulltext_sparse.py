...
cost_time = 0
def fulltext_dense_sparse_search(hybridsearch_table, question):
     global cost_time
...
def GetQuestions():
    ...
    for i in range(len(query_ids)):
        questions.append((fulltext_questions[i],dense_questions[i],sparse_questions[i],query_ids[i]))
    print(len(questions))
    return questions
    ...
    try:
        ...
        with open(path_prefix + '/three_roads/fulltext_dense_sparse_result.txt','w') as result_file:
            ...
            for question in questions:
                id += 1
                begin_time = time.time()
                qb_result, extra_result = fulltext_dense_sparse_search(hybridsearch_table, question)
                end_time = time.time()
