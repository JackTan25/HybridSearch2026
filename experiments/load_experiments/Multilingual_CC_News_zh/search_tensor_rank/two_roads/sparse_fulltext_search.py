...
import pandas as pd
from Multilingual_CC_News_zh.search.colbert_read import load_colbert_list
from utils import add_escape_characters
from Multilingual_CC_News_zh.search.vec_read import load_dense, load_sparse

path_prefix = "/home/ubuntu/hybridsearch/experiments/load_experiments/Multilingual_CC_News_zh/search_tensor_rank"
...
cost_time = 0
def fulltext_sparse_search(hybridsearch_table, question):
     global cost_time
     begin_time = time.time()
     res = (
                hybridsearch_table.output(["docid_col","fulltext_col"])
                .match_text("fulltext_col", question[0], 10)
                .match_sparse("sparse_col", question[1], "ip", 10)
...
def GetAllQuestions():
    df = pd.read_csv('/home/ubuntu/hybridsearch/experiments/load_experiments/Multilingual_CC_News_zh/queries_with_id.csv')
    query_ids = []
    fulltext_questions = []
    dense_questions = []
    sparse_questions = []
    ...
    ## dense
    dense_embedding_dir = "/home/ubuntu/data_download_data/embedding_reserve/Multilingual_CC_News_zh/query_dense_embeddings/vectors"
    ...
    while dense_file_idx < len(dense_names):
        ...
        dense_file_idx += 1
    ## sparse
    sparse_embedding_dir = "/home/ubuntu/data_download_data/embedding_reserve/Multilingual_CC_News_zh/query_sparse_embeddings/vectors"
    ...
    while sparse_file_idx < len(sparse_names):
        ...
        sparse_file_idx += 1
    ## tensor     
    tensor_embedding_dir = "/home/ubuntu/large_disk/hybridsearch/Multilingual_CC_News_zh/query_tensor_embeddings/vectors"
...
def GetQuestions():
    ...
    for i in range(len(query_ids)):
        questions.append((fulltext_questions[i],sparse_questions[i],query_ids[i],tensor_questions[i]))
    print(len(questions))
    return questions
...
def single_search(questions):
    try:
        ...
        hybridsearch_table = db_instance.get_table("Multilingual_CC_News_zh_Table")
        with open(path_prefix + '/two_roads/fulltext_sparse_result.txt','w') as result_file:
            id = 0
            time_cost = 0
            ...
            for question in questions:
                id += 1
                begin_time = time.time()
                qb_result, extra_result = fulltext_sparse_search(hybridsearch_table, question)
                end_time = time.time()
                time_cost += (end_time - begin_time) * 1000
                for i in range(len(qb_result['docid_col'])):
                    result_file.write(f"{question[2]}\t{qb_result['docid_col'][i]}\n")
                result_file.flush()
