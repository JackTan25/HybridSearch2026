...
path_prefix = "/home/ubuntu/infinity/experiments/load_experiments/Multilingual_CC_News_zh/search_tensor_rank"
...
cost_time = 0
def fulltext_sparse_search(infinity_table, question):
     global cost_time
...
def GetAllQuestions():
    df = pd.read_csv('/home/ubuntu/infinity/experiments/load_experiments/Multilingual_CC_News_zh/queries_with_id.csv')
    query_ids = []
    fulltext_questions = []
    dense_questions = []
    sparse_questions = []
...
def single_search(questions):
    try:
        ...
        infinity_table = db_instance.get_table("Multilingual_CC_News_zh_Table")
        with open(path_prefix + '/two_roads/fulltext_sparse_result.txt','w') as result_file:
            ...
            for question in questions:
                id += 1
                begin_time = time.time()
                qb_result, extra_result = fulltext_sparse_search(infinity_table, question)
                end_time = time.time()
