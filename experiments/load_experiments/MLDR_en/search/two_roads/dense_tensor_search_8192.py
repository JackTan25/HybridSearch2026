...
cost_time = 0
def dense_tensor_search(hybridsearch_table, question):
     global cost_time
...
def GetQuestions():
    ...
    for i in range(len(query_ids)):
        questions.append((dense_questions[i],tensor_questions[i],query_ids[i]))
    print(len(questions))
    return questions
...
def single_search(questions):
    ...
    try:
        ...
        with open(path_prefix + '/two_roads/dense_tensor_result_8192.txt','w') as result_file:
            id = 0
            time_cost = 0
            ...
            for question in questions:
                id += 1
                begin_time = time.time()
                qb_result, extra_result = dense_tensor_search(hybridsearch_table, question)
                end_time = time.time()
