...
path_prefix = "/home/ubuntu/hybridsearch/experiments/load_experiments/MLDR_en/search"
...
cost_time = 0
def fulltext_dense_tensor_search(hybridsearch_table, question):
     global cost_time
     ...
     res = (
                ...
                .match_tensor("tensor_col", question[2], "float", 10,
                    {'emvb_centroid_nprobe': '2','emvb_threshold_first':'0.3','emvb_threshold_final':'0.4'}
                )
...
def GetQuestions():
    ...
    for i in range(len(query_ids)):
        questions.append((fulltext_questions[i],dense_questions[i],tensor_questions[i],query_ids[i]))
    print(len(questions))
    return questions
...
def single_search(questions):
    ...
    try:
        ...
        with open(path_prefix + '/three_roads/fulltext_dense_tensor_result_8192.txt','w') as result_file:
            id = 0
            time_cost = 0
            ...
            for question in questions:
                id += 1
                begin_time = time.time()
                qb_result, extra_result = fulltext_dense_tensor_search(hybridsearch_table, question)
                end_time = time.time()
...
if __name__ == "__main__":
    ...
    with open(current_dir + "/" + current_file_name_without_extension + "_8192.time",'w') as tfile:
        tfile.write(f"{cost_time/len(questions)} ms")
        tfile.flush()
    # 读取文件内容
    file_path = '/home/ubuntu/hybridsearch/experiments/query_memory_file'
    content = read_file_content(file_path)
    with open(current_dir + "/" + current_file_name_without_extension + "_8192.memory",'w') as mfile:
        mfile.write(content)
        mfile.flush()
