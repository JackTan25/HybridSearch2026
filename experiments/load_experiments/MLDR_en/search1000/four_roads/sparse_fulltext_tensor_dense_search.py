...
path_prefix = "/home/ubuntu/hybridsearch/experiments/load_experiments/MLDR_en/search1000"
...
cost_time = 0
def fulltext_dense_sparse_tensor_search(hybridsearch_table, question):
     global cost_time
     ...
     res = (
                ...
                .match_tensor("tensor_col", question[3], "float", 1000)
...
def GetQuestions():
    ...
    for i in range(len(query_ids)):
        questions.append((fulltext_questions[i],dense_questions[i],sparse_questions[i],tensor_questions[i],query_ids[i]))
    print(len(questions))
    return questions
...
def single_search(questions):
    ...
    try:
        ...
        with open(path_prefix + '/four_roads/fulltext_dense_sparse_tensor_result.txt','w') as result_file:
            id = 0
            time_cost = 0
            ...
            for question in questions:
                id += 1
                begin_time = time.time()
                qb_result, extra_result = fulltext_dense_sparse_tensor_search(hybridsearch_table, question)
                end_time = time.time()
                time_cost += (end_time - begin_time) * 1000
                for i in range(len(qb_result['docid_col'])):
                    result_file.write(f"{question[4]}\t{qb_result['docid_col'][i]}\n")
                result_file.flush()
