...
import pandas as pd
from MS_MARCO_en.search.colbert_read import load_colbert_list
from utils import add_escape_characters
from MS_MARCO_en.search.vec_read import load_dense, load_sparse

path_prefix = "/home/ubuntu/infinity/experiments/load_experiments/MS_MARCO_en/search"
...
cost_time = 0
def dense_sparse_search(infinity_table, question):
     global cost_time
     begin_time = time.time()
     res = (
                infinity_table.output(["docid_col","fulltext_col"])
                .match_dense("dense_col", question[0] ,"float", "ip", 10, {"ef": "200"})
...
def GetAllQuestions():
    df = pd.read_csv('/home/ubuntu/infinity/experiments/load_experiments/MS_MARCO_en/queries_with_id.csv')
    query_ids = []
    fulltext_questions = []
    dense_questions = []
    sparse_questions = []
    ...
    ## dense
    dense_embedding_dir = "/home/ubuntu/data_download_data/embedding_reserve/MS_MARCO_en/query_dense_embeddings/vectors"
    ...
    while dense_file_idx < len(dense_names):
        ...
        dense_file_idx += 1
    ## sparse
    sparse_embedding_dir = "/home/ubuntu/data_download_data/embedding_reserve/MS_MARCO_en/query_sparse_embeddings/vectors"
    ...
    return (query_ids, fulltext_questions, dense_questions, sparse_questions, [])

def GetQuestions():
    # 读取 qrel 文件
    qrel_df = pd.read_csv("/home/ubuntu/infinity/experiments/load_experiments/MS_MARCO_en/qrels/test.tsv", sep='\t')
    # 按照 query-id 进行分组
    ...
    for i in range(len(query_ids)):
        if result.get(query_ids[i]) is not None:
            questions.append((dense_questions[i],sparse_questions[i],query_ids[i]))
    print(len(questions))
    return questions
...
def single_search(questions):
    try:
        ...
        infinity_table = db_instance.get_table("MS_MARCO_en_Table")
        with open(path_prefix + '/two_roads/dense_sparse_result.txt','w') as result_file:
            id = 0
            time_cost = 0
            ...
            for question in questions:
                id += 1
                begin_time = time.time()
                qb_result, extra_result = dense_sparse_search(infinity_table, question)
                end_time = time.time()
