...
cost_time = 0
def dense_tensor_search(infinity_table, question):
     global cost_time
...
def single_search(questions):
    try:
        ...
        with open(path_prefix + '/two_roads/dense_tensor_8192_result2.txt','w') as result_file:
            id = 0
            time_cost = 0
            ...
            for question in questions:
                id += 1
                begin_time = time.time()
                qb_result, extra_result = dense_tensor_search(infinity_table, question)
                end_time = time.time()
