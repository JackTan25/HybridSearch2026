...
from CQADupStack_en.vec_read import load_dense, load_sparse

path_prefix = "/home/ubuntu/infinity/experiments/load_experiments/CQADupStack_en/search1000"
...
cost_time = 0
def fulltext_dense_sparse_tensor_search(infinity_table, question):
     global cost_time
     begin_time = time.time()
     res = (
                infinity_table.output(["docid_col","fulltext_col"])
                .match_text("fulltext_col", question[0], 1000)
                .match_dense("dense_col", question[1], "float", "ip", 1000)
...
def GetAllQuestions():
    ...
    while dense_file_idx < len(dense_names):
        ...
        dense_file_idx += 1
    ## sparse
    sparse_embedding_dir = "/home/ubuntu/infinity/experiments/small_embedding/CQADupStack_en/query_sparse_embeddings/vectors"
    ...
    try:
        ...
        with open(path_prefix + '/four_roads/fulltext_dense_sparse_tensor_result.txt','w') as result_file:
            ...
            for question in questions:
                id += 1
                begin_time = time.time()
                qb_result, extra_result = fulltext_dense_sparse_tensor_search(infinity_table, question)
                end_time = time.time()
