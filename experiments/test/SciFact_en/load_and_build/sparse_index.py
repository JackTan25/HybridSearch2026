...
from SciFact_en.search_tensor_rank.three_roads.sparse_dense_fulltext_search import read_file_content, terminate_process_tree
...
try:
    ...
    infinity_table = infinity_db.get_table("SciFact_en_Table")
    infinity_table.drop_index('SciFact_en_bmp_index',ConflictType.Ignore)
    print("Start creating Bmp index...")
    begin_hnsw_time = time.time()
    res = infinity_table.create_index("SciFact_en_bmp_index", index.IndexInfo("sparse_col", index.IndexType.BMP,
                                                                            {
                                                                                "block_size": "8",
                                               ...
                                               ConflictType.Error)
    infinity_table.optimize("SciFact_en_bmp_index", {"topk": "1000"})
    end_hnsw_time = time.time()
    infinity_table.show_index("SciFact_en_bmp_index")
