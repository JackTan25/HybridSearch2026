...
try:
    ...
    infinity_table.drop_index('FiQA_2018_en_bmp_index',ConflictType.Ignore)
    print("Start creating Bmp index...")
    begin_hnsw_time = time.time()
    res = infinity_table.create_index("FiQA_2018_en_bmp_index", index.IndexInfo("sparse_col", index.IndexType.BMP,
                                                                            {
                                                                                "block_size": "8",
                                               ...
                                               ConflictType.Error)
    infinity_table.optimize("FiQA_2018_en_bmp_index", {"topk": "1000"})
    end_hnsw_time = time.time()
    infinity_table.show_index("FiQA_2018_en_bmp_index")
